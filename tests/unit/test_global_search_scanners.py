...
def test_drift_scanner_end_to_end():
    llm = MockLLM(
        [
            AIMessage(
                content="",
                tool_calls=[
                    {"name": "get_topic_news", "args": {"topic": "earnings beats raised guidance", "limit": 3}, "id": "tc1"},
                    {"name": "get_topic_news", "args": {"topic": "gap up volume expansion institutional accumulation", "limit": 3}, "id": "tc2"},
    ...
    topic_tool = SimpleNamespace(
        name="get_topic_news",
        invoke=lambda args: "continuation news" if "earnings" in args["topic"] else "gap news",
    )
    earnings_tool = SimpleNamespace(
        name="get_earnings_calendar",
        invoke=lambda args: "earnings calendar",
