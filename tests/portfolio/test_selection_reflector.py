...
from tradingagents.portfolio.selection_reflector import (
    fetch_price_data, fetch_news_summary, generate_lesson, reflect_on_scan
)
...
def test_fetch_price_data_normal(mock_yf_download):
    stock_pct, spy_pct, mfe_pct, mae_pct, top_move_dates = fetch_price_data("AAPL", "2025-01-01", "2025-01-05")

    assert stock_pct == pytest.approx(-15.0)  # (85 - 100) / 100 * 100
    assert spy_pct == pytest.approx(2.5)      # (410 - 400) / 400 * 100
...
def test_fetch_price_data_single_day(monkeypatch):
    ...
    stock_pct, spy_pct, mfe_pct, mae_pct, top_move_dates = fetch_price_data("AAPL", "2025-01-01", "2025-01-01")
    assert stock_pct is None
    assert spy_pct is None
...
def test_generate_lesson_valid():
    llm = MagicMock()
    llm.invoke.return_value = AIMessage(content='```json\n{"situation": "test sit", "advice": "test adv", "sentiment": "negative"}\n```')

    cand = {"ticker": "AAPL", "sector": "Tech", "thesis_angle": "growth", "rationale": "good", "conviction": "high"}

    lesson = generate_lesson(llm, cand, -10.0, 2.0, 5.0, -12.0, "news", 30)

    assert lesson is not None
    assert lesson["situation"] == "test sit"
    assert lesson["advice"] == "test adv"
    assert lesson["sentiment"] == "negative"

def test_generate_lesson_mfe_mae_in_prompt():
    llm = MagicMock()
    llm.invoke.return_value = AIMessage(content='{"situation": "a", "advice": "b", "sentiment": "neutral"}')

    cand = {"ticker": "AAPL"}
    generate_lesson(llm, cand, -10.0, 2.0, 5.1, -12.2, "news", 30)
...
def test_generate_lesson_bad_json():
    ...
    lesson = generate_lesson(llm, {}, -10.0, 2.0, 5.0, -12.0, "news", 30)
